# GP-16 MIDI / SysEx protocol

Developer-facing protocol notes for parsing and generating Roland GP-16 SysEx.
Application build/run and coding guidance live in [`GEMINI.md`](../GEMINI.md).
...
| Model | `0x2A` (GP-16) |
| Command | `0x11` = RQ1 (request), `0x12` = DT1 (set/data) |
| Device ID | MIDI channel − 1 (default `0x00`; range `0x00`–`0x1F`) |

RQ1 uses the same header with command `0x11`, then 3 address bytes, 3 size bytes, checksum, `F7`.

### Checksum (7-bit Roland)

1. Sum all address + data bytes (for RQ1: address + size bytes)
2. `remainder = sum % 128`
3. `checksum = remainder == 0 ? 0 : 128 − remainder`

### Inter-message delay

Between large SysEx messages, wait **20–50 ms** so the GP-16 does not overrun.

---

## Memory map

Addr0 is not a “which bank” selector — it is a bit-packed mode byte `0000 abbc`:

| Bits | Meaning |
|---|---|
| `a` | non-verifiable (0) / verifiable (1) |
| `bb` | bulk dump type: `0` Number, `1` Bank, `2` Group, `3` All |
| `c` | temporary (0) / internal (1) |

Addr1 is the patch number `0–127` (`0–63` = Group A patches 1–64, `64–127` = Group B patches 65–128). Addr2 is the parameter offset within that patch.

| Area | Addr0 (typical) |
|---|---|
| Temporary Buffer (active patch) | `00` → address `00 00 00` |
| Internal, non-verifiable, Number (bulk dumps) | `01` |
| Internal, non-verifiable, All | `07` |
| Internal, verifiable, All | `0F` |

There is no separate area byte for Group B under Number-type dumps — both groups share area `01`, distinguished by Addr1 (`00` vs `40`). Address bytes are 7-bit (`0x00`–`0x7F`); carry to the next byte when exceeding 127.

**Live capture note:** Group B RQ1 answers often arrive at `02 00 00`, not the requested `01 40 00`. Do not validate response addresses against the request — accumulate in arrival order.
...
Full table: `midi_parameter_mapping.md` Table 1. Highlights:

| Topic | Offset |
|---|---|
| Joint data Block A (5 reorderable + fixed NS) | `0x00`–`0x05` (`0x05` fixed = 5) |
| Joint data Block B (5 reorderable + fixed LOF) | `0x06`–`0x0B` (`0x0B` fixed = 11) |
| B-2 mode select | `0x0C` |
| Effect on/off + A-2 Distortion/Overdrive bit | `0x0D` / `0x0E` |
| Compressor Attack / Sustain | `0x10` / `0x11` |
| Distortion Drive | `0x14` |
| Overdrive Drive / Turbo | `0x17` / `0x18` |
...
Offsets `0x06`/`0x07`/`0x0B`/`0x0C` are **joint data / mode**, not compressor or drive parameters. Writing live edits there scrambles the effect chain.
...
Verified against committed captures:
...
Patch index comes from the buffer slot (RQ1) or `address[1]` (panel) — not from bytes at the start of the payload (those are joint data).

### Panel DT1 layout (from live capture)

```text
F0 41 <dev> 2A 12 0F <idx> 00 00  [117 data bytes]  <cs> F7
```

- Typical message length ~127 bytes; full dump ~16256 bytes
- Name: 16 ASCII chars at payload offset corresponding to `0x64` (often cited as offset 99 in raw SysEx-with-header framing — prefer address offset `0x64`)
- Effect order: first 11–12 payload bytes (joint data)
- Textbook checksum validation sometimes fails on live bulk dumps; treat layout / `0F nn` addresses as authoritative when parsing captures

Reference: `captures/dump-20260730-153932.*`.

---

## Example: Overdrive Turbo ON

```text
Address: 00 00 18
Data:    01
Checksum: sum(00+00+18+01)=19H → 128−25 = 67H
Message: F0 41 00 2A 12 00 00 18 01 67 F7
```

## Example: RQ1 temporary buffer

```text
F0 41 00 2A 11 00 00 00 00 00 46 [Checksum] F7
```

---

## Effect parameters (UI ranges)

Effects are organized into Block A and Block B. Parameters generally range **0–100** unless noted. Use Table 1 for exact MIDI offsets and bit widths.

| ID | Effect | Primary parameters | Notes |
|---|---|---|---|
| A-1 | Compressor | Sustain, Attack, Level, Tone (−50…+50) | |
| A-2a | Distortion | Drive, Tone, Level | |
...
| B-2c | Pitch Shifter | Chromatic −12…+12, Fine, Balance, Feedback | |
| B-2d | Space-D | Mode 1–4 | |
| B-3 | Auto Panpot | Rate, Depth, Mode | |
| B-4 | Tap Delay | C/L/R taps 0–1200 ms, levels, feedback, cutoff | |
| B-5 | Reverb | Decay, Mode (10 types), pre-delay, cutoff, level | |
| B-6 | Lineout Filter | Presence, Treble, Middle, Bass | Fixed last in Block B |

Joint-data effect IDs: `0`–`4` Block A reorderable, `5` Noise Suppressor; `6`–`10` Block B reorderable, `11` Lineout Filter. B-2 variant = `0x0C & 0x03`; A-2 Distortion vs Overdrive = bit in `0x0D`.

---

## Hardware overview

- 24-bit internal processing; up to 12 of 16 effect units simultaneous
- Memory: 128 patches (Group A 1–64, Group B 1–64)
- MIDI In/Out/Thru + FC-100 control port
- Unit Number (Device ID) = MIDI channel − 1


---

## Appendix: Roland Exclusive Messages / MIDI Implementation (OCR)

Raw OCR from the GP-16 MIDI Implementation booklet, retained for reference.
Prefer the curated sections above and `midi_parameter_mapping.md` when they conflict.

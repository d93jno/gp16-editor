...
namespace GP16Editor.Cli
{
    public class MidiService : IDisposable
    {
        ...
        public byte DeviceId { get; set; } = 0x10; // Default: 16 (0x10) corresponds to device ID 17 on GP-16
        ...
        public void SelectDevices(string? inputDeviceName, string? outputDeviceName)
        {
            ...
            Debug.WriteLine($"MIDI devices selected: Input='{inputDeviceName}', Output='{outputDeviceName}'");
            ...
            if (inputDevice != null && outputDevice != null)
            {
                ...
                {
                    Debug.WriteLine($"Error starting MIDI devices: {ex.Message}");
                    Console.WriteLine($"Error starting MIDI devices: {ex.Message}");
            ...
            {
                Debug.WriteLine($"MIDI devices not available: Input='{inputDeviceName}' ({inputDevice != null}), Output='{outputDeviceName}' ({outputDevice != null})");
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                try
                {
                    if (_inputDevice != null)
                    {
                        Debug.WriteLine("Stopping MIDI input device event listening");
                    ...
                    Debug.WriteLine("MIDI service disposed successfully");
                }
                catch (Exception ex)
                {
        ...
        {
            var message = new List<byte> { 0xF0, 0x41, DeviceId, 0x2A, 0x11 };
        ...
        {
            var message = new List<byte> { 0xF0, 0x41, DeviceId, 0x2A, 0x12 };
            message.AddRange(address);
            message.Add(value);
